...
As we did in the previous notebooks, we can view the image in napari using the `napari.add_image()` function. Here we set the colormap to `blue`. We additionally set the contrast limits and select a z slice where the nuclei are in focus.
